...
func (s *Service) SaveMeta(ctx context.Context, data []byte, id string, genre string) error {
	...
	res, err := retry.Retry(retry.NewAlgSimpleDefault(), 5, s.GetYTMetaFromID, ctx, id)
	if err != nil {
	...
	fileName := s.SanitizeFilename(fmt.Sprintf("%s - %s.mp3", bestMeta.Artist, bestMeta.Title))
